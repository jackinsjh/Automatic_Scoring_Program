...
#이미지 -> 문자열 추출
def ocrToStr(fullPath, outTxtPath, fileName, lang='kor'):
    #이미지 경로
    img = Image.open(fullPath)
    txtName = os.path.join(outTxtPath, fileName.split('.')[0])

    #추출(이미지파일, 추출언어, 옵션)
    outText = image_to_string(img, lang=lang, config='--psm 1 -c perserve_interword_spaces=1')
...
if __name__ == "__main__":
    OcrTxtPath = 'ocrTest4.png'
    OriImagePath = 'ocrTest4.png'
    #텍스트 파일 저장 경로
    outTxtPath = os.path.dirname(os.path.realpath(__file__)) + config['Path']['OcrTxtPath']

    for root, dirs, files in os.walk(os.path.dirname(os.path.realpath(__file__)) + config['Path']['OriImagePath']):
        for fname in files:
            fullName = os.path.join(root, fname)
            #g한글+영어 추출(kor, eng, kor+eng
...
x = 1.0
y = 1.0
img = cv2.imread('ocrTest2.jpg')  # 테스트 사진 구동 시, 파일의 확장자 jpg와 png 등 잘 확인하기!!
...
text = pytesseract.image_to_string(blur, lang='kor')  # 영어면 'euc'
print(text)
